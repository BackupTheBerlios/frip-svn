      ...
      for file in os.listdir(location):
        ...
        if os.path.isfile(fname):
          ...
          if not os.path.basename(fname) == '.svn':
            for more in self.lsf([fname], r):
              nodes.append(more)
    return nodes
  l = lsf(at, re.compile(r))
  l.sort()
  return l
  ...
  def Configure(self, conf, env):
    ...
    if os.environ.has_key('CPATH'):
      if env['PLATFORM'] == 'win32':
        ...
        sep = ':'
      for em in string.split(os.environ['CPATH'], sep):
        env.Append(CPPPATH = [em])
        print 'Found directory %s in the environment.' % (em)

    if os.environ.has_key('LPATH'):
      if env['PLATFORM'] == 'win32':
        sep = ';'
      else:
        sep = ':'
      for em in string.split(os.environ['LPATH'], sep):
